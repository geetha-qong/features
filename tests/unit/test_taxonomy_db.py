...
def test_structural_classes_not_collapsed(db_session):
    """Arrow/connector classes share (entity_class, sub_class) = (None, None)
    but must each get their own row (keyed on yolo_label in the upsert)."""
    sync_taxonomy_to_db(db_session)

    structural = (
        db_session.query(models.LabelTaxonomy)
        .filter_by(entity_class=None, sub_class=None)
        .all()
    )
