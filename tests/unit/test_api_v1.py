...
# ── GET /api/v1/jobs (list / dashboard) — OCR Done-gate ────────────────────────

def _list_status_for(client, full_key, job_id):
    resp = client.get("/api/v1/jobs", headers={"Authorization": f"Bearer {full_key}"})
    assert resp.status_code == 200
    jobs = {j["id"]: j for j in resp.json()["jobs"]}
    return jobs[job_id]["status"]


def test_list_done_job_with_null_ocr_status_reads_done(client, db_session, user, api_key_pair):
    """Regression: a legacy job (done, ocr_status NULL — OCR never enqueued) must
    NOT be masked as 'processing'. The Done-gate only holds jobs whose OCR is
    actively pending; NULL means OCR was never applicable."""
    job = models.Job(user_id=user.id, pid_no="T-100", status="done",
                     original_filename="legacy.pdf", stored_filename="legacy.pdf",
                     ocr_status=None)
    db_session.add(job)
    db_session.commit()

    full_key, _ = api_key_pair
    assert _list_status_for(client, full_key, job.id) == "done"


def test_list_done_job_with_pending_ocr_reads_processing(client, db_session, user, api_key_pair):
    """A freshly-uploaded job whose OCR is still running shows 'processing' so the
    card stays 'Preparing' until OCR-resolved tags are ready (#132 intent)."""
    job = models.Job(user_id=user.id, pid_no="T-101", status="done",
                     original_filename="fresh.pdf", stored_filename="fresh.pdf",
                     ocr_status="pending")
    db_session.add(job)
    db_session.commit()
...
def test_list_done_job_with_ocr_done_reads_done(client, db_session, user, api_key_pair):
    job = models.Job(user_id=user.id, pid_no="T-102", status="done",
                     original_filename="ready.pdf", stored_filename="ready.pdf",
                     ocr_status="done")
    db_session.add(job)
    db_session.commit()
...
# ── Cross-tile dedup in /detections endpoint ───────────────────────────────────

def test_detections_dedups_cross_tile_duplicates(client, db_session, user, api_key_pair, tmp_path):
    """The /detections endpoint must collapse same-class detections that map
    to the same page-space bbox from two overlapping tiles.

    Tile geometry for a 900x900 page (GRID=3x3, OVERLAP=20%):
      tile_p0_r0_c0.png: x0=0   -> bbox [280,5,300,25] maps to page [280,5,300,25]
      tile_p0_r0_c1.png: x0=240 -> bbox [ 40,5, 60,25] maps to page [280,5,300,25]
    IoU = 1.0 -> dedupe_detections_page_space must keep only 1 (higher confidence).
    """
    import json as _json
    out_dir = tmp_path / "job_dir"
    out_dir.mkdir()
    # canonical_graph.json provides page dimensions for page_dims_for_job
    (out_dir / "canonical_graph.json").write_text(
        _json.dumps({"page_width": 900, "page_height": 900})
    )
    csv_path = out_dir / "output.csv"
    csv_path.write_text("x")

    # Two detections of the SAME class whose tile-local bboxes land on the
    # identical page-space rectangle (IoU=1.0).
    dets = [
        {
            "label": "valve_bv",
            "tile": "tile_p0_r0_c0.png",
    ...
    assert resp.status_code == 200
    result_dets = resp.json()["detections"]
    # Dedup MUST have collapsed the two same-class overlapping detections to 1.
    assert len(result_dets) == 1, (
        f"Expected 1 detection after cross-tile dedup, got {len(result_dets)}: {result_dets}"
    )
    # The higher-confidence detection (0.9, from c1) should be the survivor.
    assert result_dets[0]["confidence"] == 0.9
