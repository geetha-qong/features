...
def run_pipeline_for_job_rq(
    ...
    try:
        ...
        # Pipe/line detection — runs after YOLO so symbol bboxes are available
        # for Phase 2 masking. Writes graph_outputs/topology.json (line coords
        # only, no pixel arrays) that the tile-analysis endpoint reads.
        # Non-fatal — topology overlay is additive; CSV deliverable unaffected.
        try:
            _run_line_detection(job_id, job_dir)
        except Exception as _ld_err:
            print(f"[line-detect] job {job_id}: {_ld_err}", file=sys.stderr)

        # Graph extraction + enrichment (FEATURES #77).
        # graph_status tracks this stage independently of the CSV pipeline:
        #   pending  → not started yet
        #   running  → extract_graph / enrich_graph / pass3 / apply_enrichment in progress
        #   done     → all graph steps completed successfully
        #   failed   → any step threw; overall job status also becomes "failed"
        # Job is openable and all CSV deliverables remain accessible even when failed.
        _graph_ok = False
        ...
        try:
            ...
            if detections:
                ...
                from webapp.graph.enrichment import enrich_graph, apply_enrichment_to_canonical
                _enrich_stats = enrich_graph(str(job_dir))
                print(f"[graph-enrich] job {job_id}: "
                      f"wet={_enrich_stats.get('wet')} dry={_enrich_stats.get('dry')} "
                      f"interlock={_enrich_stats.get('interlock')} "
                      f"alarms=HH:{_enrich_stats.get('alarm_hh')} H:{_enrich_stats.get('alarm_h')} "
                      f"L:{_enrich_stats.get('alarm_l')} LL:{_enrich_stats.get('alarm_ll')} "
                      f"ocr={_enrich_stats.get('ocr_available')}")

                # Pass 3: Vision API tag lookup for untagged pump/motor nodes.
                # Non-fatal within graph stage — a tag-lookup failure does not
                # fail the whole graph; it just leaves those nodes untagged.
                try:
                    from extractor import apply_third_pass
                    _p3_stats = apply_third_pass(str(job_dir))
                    print(f"[graph-pass3] job {job_id}: "
                          f"untagged_before={_p3_stats.get('untagged_before')} "
                          f"tagged={_p3_stats.get('tagged')}")
                except Exception as _p3_err:
                    print(f"[graph-pass3] job {job_id}: {_p3_err}", file=sys.stderr)

                try:
                    from extractor import apply_fourth_pass
                    _p4_stats = apply_fourth_pass(str(job_dir))
                    print(f"[graph-pass4] job {job_id}: "
                          f"lines_found={_p4_stats.get('lines_found')} "
                          f"tiles_queried={_p4_stats.get('tiles_queried')} "
                          f"skipped={_p4_stats.get('skipped')}")
                except Exception as _p4_err:
                    print(f"[graph-pass4] job {job_id}: {_p4_err}", file=sys.stderr)

                # Pass 4.5: orientation-aware line-number coverage pass.
                # Flag-gated — unset => identical behaviour/API-usage/output.
                # Gemini Vision stays the only OCR engine; the tracer only
                # selects better crops/orientations (FEATURES #115, spec
                # docs/superpowers/specs/2026-06-26-orientation-aware-line-ocr-design.md).
                if os.environ.get("LINE_ORIENT_OCR") == "1":
                    try:
                        ...
                        print(f"[graph-pass4.5] job {job_id}: "
                              f"traced={_po.get('segments_traced')} "
                              f"calls={_po.get('vision_calls')} "
                              f"new_lines={_po.get('new_line_numbers')} "
                              f"skipped={_po.get('skipped')}")
                    except Exception as _po_err:
                        print(f"[graph-pass4.5] job {job_id}: {_po_err}", file=sys.stderr)

                try:
                    from extractor import apply_fifth_pass
                    _p5_stats = apply_fifth_pass(str(job_dir))
                    print(f"[graph-pass5] job {job_id}: "
                          f"found={_p5_stats.get('found')} "
                          f"queried={_p5_stats.get('queried')} "
                          f"failed={_p5_stats.get('failed')} "
                          f"skipped={_p5_stats.get('skipped')}")
                except Exception as _p5_err:
                    print(f"[graph-pass5] job {job_id}: {_p5_err}", file=sys.stderr)

                _canon_stats = apply_enrichment_to_canonical(str(job_dir), job_id, db)
                print(f"[graph-enrich-canon] job {job_id}: {_canon_stats}")

                job.graph_status = "done"
                db.commit()
                _graph_ok = True
            else:
            ...
            job.status = "failed"
            job.error_msg = f"Graph enrichment failed: {_g_err}"
            db.commit()

        # Text index for drawing search (FEATURES #XX — notes/holds/annotations).
        # Non-fatal; CSV deliverables are unaffected by failure.
        try:
            from webapp.graph.text_index import extract_and_save as _extract_text
            _n_pages = _extract_text(str(job_dir))
            print(f"[text-index] job {job_id}: {_n_pages} pages indexed")
        except Exception as _ti_err:
            print(f"[text-index] job {job_id}: {_ti_err}", file=sys.stderr)

        final_run_status = "done" if _graph_ok else "failed"
        _finalize_job_run(run_id, final_run_status)
        ...
        # Enqueue OCR as part of every pipeline run so the dashboard "Done"
        # gate works regardless of whether the user ever opens Studio.
        try:
            ...
            print(f"[ocr-enqueue] job {job_id}: queued")
        except Exception as _ocr_err:
            print(f"[ocr-enqueue] job {job_id}: {_ocr_err}", file=sys.stderr)
    finally:
        db.close()
...
def _detections_are_stale(detections) -> bool:
    """True if stored ``gpu_detections`` came from an obsolete model — i.e. any
    label is outside the *current* taxonomy (e.g. ``valve_gen``/``valve_gl``,
    dropped in the 23-class overhaul).

    The live Windows GPU worker still writes the LEGACY *schema*
    (``yolo_class``/``bbox_tile``) but with CURRENT taxonomy names, so schema
    shape alone is NOT a staleness signal — only an out-of-taxonomy label is.
    Detections with no label at all are left untouched (can't prove obsolete).
    """
    if not detections:
        return False
    try:
    ...
    except Exception:  # noqa: BLE001 — taxonomy unavailable → don't force re-infer
        return False
    for det in detections:
        label = det.get("label") or det.get("yolo_class")
        if label is not None and label not in current:
            return True
    return False
...
def _run_inplace_inference(job_id: int, job_dir: Path) -> None:
    ...
    try:
        ...
        if not job:
            return
        # Don't clobber a real GPU-worker result that already landed — UNLESS the
        # stored rows are STALE (obsolete-taxonomy labels from a dead model).
        # Re-inferring stale rows heals legacy jobs whose graph was built from a
        # few old-model boxes (job 43: 7 stale `valve_gen`/`valve_gl` vs 268 live).
        if job.gpu_detections:
            try:
                existing = json.loads(job.gpu_detections)
                if isinstance(existing, list) and len(existing) > 0:
                    if _detections_are_stale(existing):
                        print(f"[inplace-yolo] job {job_id}: {len(existing)} stored detections "
                              f"use obsolete taxonomy — re-inferring")
                    else:
...
def _run_line_detection(job_id: int, job_dir: Path) -> None:
    """Detect pipe/line segments for all tiles and write graph_outputs/topology.json.

    Runs after _run_inplace_inference so Job.gpu_detections is populated and
    YOLO bboxes can be used to mask symbol regions (Phase 2 of line_detector).
    Reads gpu_detections in its own short DB session, then calls line_detector
    which is pure CPU/filesystem work — no session held during heavy lifting.
    """
    from webapp.line_detector import detect_lines_for_job

    db = _short_session()
    try:
        job = db.query(models.Job).filter(models.Job.id == job_id).first()
        gpu_detections_json = job.gpu_detections if job else None
    finally:
        db.close()

    detect_lines_for_job(job_id, job_dir, gpu_detections_json)
