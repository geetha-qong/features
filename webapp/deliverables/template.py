...
class ColumnDef(BaseModel):
    ...
    `group` is an optional group header (e.g. "AT WORKING CONDITIONS") that
    spans this column and siblings with the same group string. The XLSX
    generator renders a 2-row header: row 1 = group (merged), row 2 = header.
    Columns without a group span both rows (merged vertically).
    """
