...
class TemplateLoader:
    def __init__(self, template_dir: Path = TEMPLATE_DIR) -> None:
        ...
        self._raw_cache: Dict[str, Any] = {}  # slug → (mtime, raw_dict)

    def load(self, slug: str) -> TemplateConfig:
        # Always call _load_raw first — it invalidates _cache when mtime changes.
        raw = self._load_raw(slug)
        if slug not in self._cache:
            self._cache[slug] = TemplateConfig.model_validate(raw)
    ...
    def _load_raw(self, slug: str) -> Dict[str, Any]:
        ...
        Cache is invalidated automatically when the file's mtime changes, so
        edits to template JSON are picked up without a server restart.
        """
        path = self._template_dir / f"{slug}.json"
        if not path.exists():
            raise TemplateNotFound(slug)
        mtime = path.stat().st_mtime
        cached_mtime, cached_data = self._raw_cache.get(slug, (None, None))
        if cached_data is None or cached_mtime != mtime:
            cached_data = json.loads(path.read_text())
            self._raw_cache[slug] = (mtime, cached_data)
            # Invalidate the parsed TemplateConfig cache too so load() re-parses.
