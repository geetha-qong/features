...
def _classify_system(type_code: str, instrument_description: str) -> str:
    ...
    if code in _SIS_TYPE_CODES:
        return "SIS"
    # Word-boundary match to avoid "SIS" matching inside "ANALYSIS", etc.
    desc_words = set(re.split(r'\W+', desc))
    if desc_words & _SIS_DESC_KEYWORDS:
        return "SIS"
    ...
    with valve_csv.open(newline="", encoding="utf-8-sig") as f:
        ...
        for idx, row in enumerate(reader):
            ...
            fields = {
                ...
                "fluid_type": row.get("Fluid Type", ""),
                "phase": row.get("Phase", ""),
                "service_description": row.get("Service Description", ""),
                "design_temp": row.get("Design Temp", ""),
                "design_press": row.get("Design Press", ""),
                "piping_rating": row.get("Piping Rating", ""),
...
def _build_instrument_entities(instrument_csv: Path, job_id: int) -> List[CanonicalEntity]:
    ...
    with instrument_csv.open(newline="", encoding="utf-8-sig") as f:
        ...
        for idx, row in enumerate(reader):
            ...
            # Unit Number: leading numeric segment (e.g. "62" from "62-FE-151002B")
            ...
            # Loop Number: replace the type code with its first letter, keeping
            # the rest of the tag — INCLUDING any trailing instance letter — so
            # the loop number preserves the full sequence (A/B kept distinct):
            #   "FI-8025"  → "F-8025"    |  "LS-8057A" → "L-8057A"
            #   "PT-8077A" → "P-8077A"   |  "PT-8077B" → "P-8077B"
            #   "62-LIC-151006" → "62-L-151006"
            ...
            # Transmitter / primary-element fallback: 4-20mA when vendor API has no data
            if not fields.get("io_output") and fields.get("io_type") == "AI":
                fields["io_output"] = "4-20mA"

            # Store extracted manufacturer/model as plain fields only.
            # vendor_match is intentionally left None — it is only populated
            # when the user explicitly accepts a vendor in the UI (via entity_overrides).
            ...
            if manufacturer not in _EMPTY_VENDOR_VALUES:
                fields["manufacturer"] = manufacturer
            if model_no not in _EMPTY_VENDOR_VALUES:
                fields["model_no"] = model_no
...
_KNOWN_EQUIPMENT_TYPES = {
    "pump", "motor", "compressor", "heat_exchanger",
    "reducer", "expander", "eccentric", "flange",
    "vessel", "strainer", "other",
}


def _build_equipment_entities(equipment_csv: Path, job_id: int) -> List[CanonicalEntity]:
    """Parse equipment_list.csv and return CanonicalEntity objects with entity_class='equipment'."""
    entities: List[CanonicalEntity] = []
    with equipment_csv.open(newline="", encoding="utf-8-sig") as f:
        reader = csv.DictReader(f)
        for idx, row in enumerate(reader):
            tag = (row.get("Tag") or "").strip() or None
            name = (row.get("Equipment Name") or "").strip()
            etype = (row.get("Equipment Type") or "other").strip().lower()
            pid_number = (row.get("P&ID No") or "").strip()

            if etype not in _KNOWN_EQUIPMENT_TYPES:
                etype = "other"

            # Derive unit_number from the area-code prefix of the tag.
            # Tags like "62-P-151005" → unit_number = "62".
            unit_number = ""
            if tag:
                first = tag.split("-")[0]
                if first.isdigit():
            ...
            entity_id = _make_entity_uuid(job_id, "equipment", tag, idx)
            entities.append(
                CanonicalEntity(
                    entity_id=entity_id,
...
def write_canonical_for_job(
    ...
    Reads valve_list.csv, instrumentation_index.csv, and equipment_list.csv if
    present (missing files are silently skipped). Writes canonical.json into the
    ...
    equipment_csv = job_dir / "equipment_list.csv"
    if equipment_csv.exists():
        entities.extend(_build_equipment_entities(equipment_csv, job_id))
    ...
    equipment_count = sum(1 for e in entities if e.entity_class == "equipment")
