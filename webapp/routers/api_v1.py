        ...
        "jobs": [
            {
                ...
                "status": _effective_status(j),
                "graph_status": j.graph_status,
...
def _attach_entity_ids(detections: list, entities: list) -> None:
    ...
    for e in entities:
        if e.tag:
            ...
            id_to_tag[str(e.entity_id)] = e.tag
    consumed: set = set()
    ...
    for det in detections:
        ...
        # Step 1+2: tag-equality matches. Candidates in priority order:
        # - entity_tag: set by instrument OCR (most reliable, use first)
        # - valve_tag: legacy GPU worker output
        # - tag: generic tag field
        # - label: when the label string happens to be tag-shaped (e.g. fixtures)
        for candidate_tag in (det.get("entity_tag"), det.get("valve_tag"), det.get("tag"), det.get("label")):
        ...
        if entity_id is None:
            ...
            if cls:
                ...
                if entity_id is None and sub:
                    # Only fall back to class-only when canonical uses a
                    # different sub_class code system entirely (e.g. YOLO 'BV'
                    # vs canonical 'VB'). If any canonical entity already
                    # carries sub_class == sub (same code system, just all
                    # consumed), excess detections must NOT steal entities of
                    # a different sub_class (e.g. extra pump detections must
                    # not consume motor entities).
                    same_system = any(
                        e.entity_class == cls
                        and (e.sub_class or "").upper() == sub
                        for e in entities
                    )
                    # Also: if ANY entity in the same class has a non-null
                    # sub_class (even a different one), keep matching strict.
                    # Without this guard Motor detections steal Pump entities
                    # when there are no Motor canonical entries.
                    class_has_sub = any(
                        e.entity_class == cls and e.sub_class
                        for e in entities
                    )
                    if not same_system and not class_has_sub:
                        for e in entities:
                            eid = str(e.entity_id)
                            if eid in consumed or e.entity_class != cls:
                                continue
        ...
        det["entity_tag"] = id_to_tag.get(entity_id) if entity_id else None
...
async def api_job_detections(
    ...
    # Cross-tile dedup: collapse same-class detections that map to the same
    # page-space bbox from overlapping tiles (IoU >= 0.5).  Gate on page dims
    # being available (canonical_graph.json present) — skip safely if absent.
    if detections and job.output_csv_path:
        ...
        if _dims:
            detections = dedupe_detections_page_space(
                detections, compute_tile_offsets(_dims[0], _dims[1], 0)
            )
...
# ── GET /api/v1/jobs/{job_id}/search/text ─────────────────────────────────────
...
def job_text_search(
    ...
    """Full-text search over free-form drawing text (notes, holds, annotations)."""
    if not q or len(q) < 2:
        return {"results": []}
    job = db.query(models.Job).filter(models.Job.id == job_id).first()
    if not job:
    ...
    from webapp.graph.text_index import search as text_search
    job_dir = str(get_job_dir(job))
    results = text_search(job_dir, q)
...
def _validate_datasheet_url(url: str) -> None:
    """Reject URLs that could reach internal/cloud-metadata endpoints (SSRF guard)."""
    import ipaddress
    import socket
    from urllib.parse import urlparse
...
@router.get("/datasheet-pdf")
async def datasheet_pdf_proxy(
    url: str,
    _: models.User = Depends(get_current_user),
):
    """Fetch a datasheet PDF from an external vendor server and stream it to the browser."""
    import httpx
    from fastapi.responses import StreamingResponse
    import logging

    _validate_datasheet_url(url)

    try:
        async with httpx.AsyncClient(timeout=30, follow_redirects=False) as client:
            r = await client.get(url)
        if r.status_code != 200:
            raise HTTPException(status_code=502, detail=f"Datasheet server returned {r.status_code}")
        content_type = r.headers.get("content-type", "")
        if "pdf" not in content_type.lower():
            raise HTTPException(status_code=502, detail="Upstream response is not a PDF")
        ...
        logging.getLogger(__name__).error("datasheet_pdf_proxy error: %s", exc)
        raise HTTPException(status_code=502, detail="Could not fetch datasheet")


@router.post("/jobs/{job_id}/export/datasheet/xlsx")
async def export_datasheet_xlsx(
    job_id: int,
    active_sheet: Optional[str] = Query(None, description="Tag of the sheet to open first"),
    db: Session = Depends(get_db),
    current_user: models.User = Depends(_get_api_user),
):
    """Export datasheet Excel for a job with all user edits (overrides) applied."""
    import io
    ...
    from webapp.deliverables.instruments.datasheet import DatasheetXLSXGenerator
    from webapp.deliverables.template_loader import TemplateLoader

    job = db.query(models.Job).filter(models.Job.id == job_id).first()
    if not job or job.user_id != current_user.id and current_user.role != "super_admin":
        raise HTTPException(status_code=404, detail="Job not found")
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="Canonical data not found")

    loader = TemplateLoader()
    template = loader.load_with_fallback(canonical.customer_template_slug)

    generator = DatasheetXLSXGenerator()
    xlsx_bytes = generator.generate(canonical, template, active_sheet=active_sheet)

    return StreamingResponse(
        iter([xlsx_bytes]),
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f'attachment; filename="datasheet-{job_id}.xlsx"'},
    )
