...
from webapp.graph.loader import compute_tile_offsets, to_page_pixel_detections
...
class AnnotationCreate(BaseModel):
    ...
    def _entity_class_valid(cls, v: str) -> str:
        if v not in {"valve", "instrument", "equipment", "annotation"}:
            raise ValueError(
                "entity_class must be one of 'valve', 'instrument', 'equipment', 'annotation'"
            )
        return v
...
def _parse_raw_detections(job: Job) -> List[Dict[str, Any]]:
    """Parse Job.gpu_detections JSON → list of detection dicts (empty on failure)."""
    raw = job.gpu_detections
    if not raw:
        return []
    try:
...
def _page_dims_from_canonical_graph(job: Job) -> Optional[Tuple[int, int]]:
    """Read page_width/page_height from canonical_graph.json next to the CSV.

    Returns (width, height) as ints, or None if the file is absent or lacks
    the keys. The annotations router uses this to drive tile-offset computation.
    """
    if not job.output_csv_path:
        return None
    graph_path = Path(job.output_csv_path).parent / "canonical_graph.json"
    ...
    try:
        ...
        return None
    pw = data.get("page_width")
    ph = data.get("page_height")
    if pw and ph:
        try:
...
def _resolve_detection_index_by_bbox(
    job: Job,
    page_bbox: List[float],
    iou_threshold: float = 0.5,
) -> int:
    """Return the gpu_detections index whose page-pixel bbox best IoU-matches
    ``page_bbox`` (≥ ``iou_threshold``), or -1 if none qualifies.

    Resolution is done entirely server-side so tile-offset translation is
    correct regardless of which tile the detection lives in — client-side
    resolution only worked for the top-left tile because detection bboxes in
    ``Job.gpu_detections`` are tile-local.

    Steps:
      1. Parse raw detections via ``_parse_raw_detections`` (preserves list
         order so the returned index maps back to the original array).
      2. Determine page dims from ``canonical_graph.json``; return -1 if absent.
      3. ``compute_tile_offsets`` → ``to_page_pixel_detections`` to get each
         detection's page-pixel bbox.
      4. IoU-match ``page_bbox`` against every page-det bbox; return the index
         with the best IoU ≥ threshold, else -1.
    """
    raw_dets = _parse_raw_detections(job)
    if not raw_dets:
        return -1

    dims = _page_dims_from_canonical_graph(job)
    if dims is None:
        return -1
    w, h = dims

    offsets = compute_tile_offsets(w, h, page_index=0)
    page_dets = to_page_pixel_detections(raw_dets, offsets)

    best_idx = -1
    best_iou = 0.0
    for i, det in enumerate(page_dets):
        det_bbox = det.get("bbox")
        if not det_bbox or len(det_bbox) != 4:
            continue
        v = bbox_iou(page_bbox, det_bbox)
        if v > best_iou:
            best_iou = v
            best_idx = i
...
def create_annotation(
    ...
    # Server-side detection claim resolution — only when the client didn't
    # already supply an explicit index.  Translates tile-local bboxes to
    # page-pixel (via canonical_graph.json dimensions) before IoU-matching so
    # the result is correct for ANY tile, not just the top-left one.
    if payload.linked_detection_index is None:
        resolved = _resolve_detection_index_by_bbox(job, bbox)
        if resolved >= 0:
            payload = payload.model_copy(update={"linked_detection_index": resolved})
