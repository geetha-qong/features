...
from webapp.deliverables.line_list import generate_line_list
...
async def serve_page_full(
    ...
    **High-DPI on demand (FEATURES #41):** the studio canvas measures the
    ...
    if not full_path.exists():
        # Auto-regenerate tiles from input.pdf if tmp was wiped
        input_pdf = get_job_dir(job) / "input.pdf"
        if input_pdf.exists():
            try:
                from pdf_to_tiles import pdf_to_tiles as _pdf_to_tiles
                _pdf_to_tiles(str(input_pdf), output_dir=str(tmp_dir))
            except Exception:
                pass
    if not full_path.exists():
...
async def rerun_job(
    ...
    # Clear stored detections so the re-run does FRESH YOLO inference. Without
    # this, _run_inplace_inference's skip-guard sees the old rows and skips,
    # rebuilding the graph from stale boxes (job 43: re-process kept 7 old
    # detections instead of the 268 the current model finds).
    job.gpu_detections = None
...
@router.get("/jobs/{job_id}/line-list")
async def get_line_list(
    ...
    rows = generate_line_list(job_dir, job_id, db)
