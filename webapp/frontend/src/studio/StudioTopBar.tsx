...
interface Props {
  ...
  onBulkReview: () => void;
  onAllData: () => void;
  searchEntities: EntityRow[];
  onSearchJump: (entityId: string, entityClass: string) => void;
  onSearchResultsChange: (entityIds: string[]) => void;
  onSearchActiveChange: (entityId: string | null) => void;
  onSearchJumpToSheet: (pageIndex: number) => void;
}
...
const DELIVERABLES: { type: string; format: "csv" | "xlsx"; label: string; ext: string }[] = [
  ...
  { type: "equipment_list",    format: "csv",  label: "Equipment List · CSV",     ext: "csv"  },
  { type: "equipment_list",    format: "xlsx", label: "Equipment List · XLSX",    ext: "xlsx" },
  { type: "io_list",           format: "csv",  label: "I/O List · CSV",           ext: "csv"  },
  { type: "io_list",           format: "xlsx", label: "I/O List · XLSX",          ext: "xlsx" },
  { type: "datasheet",         format: "xlsx", label: "Datasheet · XLSX",         ext: "xlsx" },
  { type: "line_list",         format: "csv",  label: "Line List · CSV",           ext: "csv"  },
  { type: "line_list",         format: "xlsx", label: "Line List · XLSX",          ext: "xlsx" },
];
  ...
  return (
    <header className="studio-top">
      ...
      <SearchBar
        ...
        onJumpToSheet={onSearchJumpToSheet}
      />
      ...
      <div className="more-wrap">
        ...
        {showMenu && (
          <div className="more-menu" onMouseLeave={() => { setShowMenu(false); setShowExport(false); }}>
            ...
            <button onClick={() => { setShowMenu(false); navigate("/account"); }}>
