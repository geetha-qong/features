...
interface Props {
  ...
  onBulkReview?: () => void;
  totalCount?: number;
  /** Per-deliverable entity counts — keyed by deliverable_type string.
   *  When provided, the Bulk Review badge shows the count for the active tab
   *  instead of the static totalCount. */
  ...
  /** Fired after any successful save (tag edit, field draft, or create) so the
   *  parent can invalidate caches + re-pull entities — keeps the canvas panel /
   *  sidebar in sync with the just-persisted EntityOverride. Without it the
   *  parent shows the pre-edit value until a hard reload. */
  onSaved?: () => void;
}
          ...
          <div className="ds-head-top">
            ...
            {onBulkReview && (
              ...
              >
                ...
                <span className="num">{entityCounts?.[deliverableType ?? ""] ?? totalCount}</span>
              </button>
            )}
              ...
              {tagEditMode ? (
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}>
                  <input
                    ...
                    onKeyDown={async (e) => {
                      if (e.key === "Enter") {
                        ...
                        setTagError(null);
                        try {
                          await patchEntity(jobId, entityId, { tag: trimmed });
                          setDsSections((prev) => prev ? { ...prev, tag: trimmed } : prev);
                          setOriginalEntity((prev) => prev ? { ...prev, tag: trimmed } : prev);
                          setTagEditMode(false);
                          setToast({ kind: "ok", msg: "Tag saved" });
                          onSaved?.();
                        } catch (err) {
                    ...
                    style={{
                      ...
                      border: "none",
                      borderBottom: "2px solid var(--accent, #00e5ff)",
                  ...
                  <button
                    ...
                    style={{ background: "none", border: "none", padding: 2, cursor: "pointer", color: "var(--ok, #4caf50)", opacity: tagSaving ? 0.5 : 1 }}
                  >
                  ...
                  <button
                    ...
                    title="Cancel (Esc)"
                    disabled={tagSaving}
                    onClick={() => { setTagEditMode(false); setTagError(null); }}
                    style={{ background: "none", border: "none", padding: 2, cursor: "pointer", color: "var(--fg-2, rgba(255,255,255,0.55))" }}
                  >
                    <X size={16} strokeWidth={2} />
                  </button>
                  {tagError && (
                    <span style={{ fontSize: 11, color: "var(--error, #ff6b6b)", marginLeft: 4 }}>{tagError}</span>
                  )}
                </div>
              ) : (
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <h2 style={{ margin: 0 }}>{headerTag}</h2>
                  {entityId && (
                    <button
                      type="button"
                      title="Edit tag"
                      onClick={() => { setTagDraft(headerTag); setTagEditMode(true); setTagError(null); }}
                      style={{ background: "none", border: "none", padding: 2, cursor: "pointer", color: "var(--accent, #00e5ff)", opacity: 0.7, flexShrink: 0 }}
                    >
          ...
          <div className="ds-fields">
            {readOnly.map((col) => (
              <ReadOnlyCell
                key={col.field}
                col={col}
                fv={entity?.values[col.field]}
                span={spanForField(col.header)}
              />
            ))}
          </div>
        </section>
