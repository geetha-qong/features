...
export interface DetectionItem {
  ...
  entity_class?: string;
  /** Engineering tag extracted from the matched canonical entity (e.g. "62-GL-151006").
   *  Shown as the bbox label instead of the raw YOLO class name. */
...
async function call<T>(path: string): Promise<T> {
  const res = await fetch(path, { credentials: "include", redirect: "manual", cache: "no-cache" });
...
export interface DetectionsTaggedResp {
  detections: DetectionItem[];
  ocr_run: boolean;
  /** "ready" = tags resolved; "empty" = nothing to OCR (terminal);
   *  "pending" = background OCR enqueued/running, poll again. Absent on
   *  legacy cached responses (treat as "ready"). */
  ocr_status?: "ready" | "empty" | "pending";
}

/** Fetch detections with entity_id resolved by OCR-ing inst_field tag bubbles
 *  and propagating the matched entity to the nearest symbol bbox.
 *  Served from cache after first run. Pass refresh=true to recompute. */
export const getDetectionsTagged = (jobId: number, refresh = false) =>
  call<DetectionsTaggedResp>(
    `/api/v1/jobs/${jobId}/detections-tagged${refresh ? "?refresh=true" : ""}`,
  );
...
export interface DatasheetFieldOut {
  ...
  value: unknown;
  is_override: boolean;
  options?: string[];   // if present, render as a dropdown select
  group?: string;       // consecutive same-group fields render as one unified grouped container
}
...
/** Mark an auto-detected graph edge as rejected by the user (hides it from the graph). */
export const rejectAutoEdge = (jobId: number, autoEdgeId: string): Promise<void> =>
  callJson<void>("POST", `/api/v1/jobs/${jobId}/graph/edges/${encodeURIComponent(autoEdgeId)}/reject`, {}).then(() => undefined);

/** Undo a previous auto-edge rejection (restore the edge). */
export const unrejectAutoEdge = (jobId: number, autoEdgeId: string): Promise<void> =>
  callJson<void>("DELETE", `/api/v1/jobs/${jobId}/graph/edges/${encodeURIComponent(autoEdgeId)}/reject`).then(() => undefined);

// ──────────────────────────────────────────────────────────────────────────────
// Node corrections (2026-06-28 design) — soft-reject a detected graph node and
// undo it. Detected (source:"auto") nodes are rejected via these endpoints;
// user-added (source:"user") nodes are hard-deleted via deleteAnnotation instead.
// Both endpoints key on the canonical `entity_id` (the cross-store spine), NOT
// the post-dedup graph node_id. Idempotent on the backend (re-reject = 200
// no-op; undo of a non-reject = 404, treated as already-undone).
// ──────────────────────────────────────────────────────────────────────────────

/** Soft-reject a detected graph node (records a `__rejected__` entity override).
 *  Drops it (and its dangling edges) from GET /graph and from the deliverables,
 *  reversibly. POST returns 200. */
export const rejectNode = (jobId: number, entityId: string): Promise<void> =>
  callJson<void>(
    "POST",
    `/api/v1/jobs/${jobId}/nodes/${encodeURIComponent(entityId)}/reject`,
    {},
  ).then(() => undefined);

/** Undo a previous node rejection (restore the detected node). DELETE returns
 *  204; a 404 means it wasn't rejected (already undone) and is swallowed so the
 *  Restore/Undo affordance is forgiving. */
export const unrejectNode = (jobId: number, entityId: string): Promise<void> =>
  callJson<void>(
    "DELETE",
    `/api/v1/jobs/${jobId}/nodes/${encodeURIComponent(entityId)}/reject`,
  )
...
export const searchDrawingText = (jobId: number, q: string) =>
  callJson<TextSearchResponse>(
    "GET",
    `/api/v1/jobs/${jobId}/search/text?q=${encodeURIComponent(q)}`,
  );
