...
describe("GraphLayer", () => {
  ...
  test("each node has an enlarged transparent hit target larger than the visible dot", () => {
    // Regression: on large drawings the visible dot renders ~2-3px at fit-zoom,
    // making type-B nodes unclickable (so they can't be adopted). A transparent
    // hit circle, sized independently of the dot, restores a usable click target.
    // Node placed away from the top-left margin guard (cx/pw>=0.07, cy/ph>=0.08).
    const graph = makeGraph({
      nodes: [
        { id: "n_hit", entity_id: null, tag: null, class: "valve_bv", bbox: [200, 200, 240, 240], tile: "t0", confidence: 0.5 },
      ],
      edges: [],
      floating_nodes: [],
    });
    const { container } = renderLayer({ graph });
    const visible = container.querySelector('[data-graph-node="n_hit"]') as SVGCircleElement;
    expect(visible).not.toBeNull();
    const g = visible.closest("g")!;
    const hit = [...g.querySelectorAll("circle")].find(
      (c) => c.getAttribute("fill") === "transparent",
    ) as SVGCircleElement | undefined;
    expect(hit).toBeTruthy();
    expect(parseFloat(hit!.getAttribute("r")!)).toBeGreaterThan(
      parseFloat(visible.getAttribute("r")!),
    );
