...
export default function QongDrive() {
  const [progress, setProgress] = useState<DriveProgress>({ ins: 0, val: 0, pip: 0 });
  ...
  const tagPill = (color: string): React.CSSProperties => ({
    fontFamily: "var(--font-mono)",
    fontSize: 10,
    fontWeight: 600,
    padding: "3px 7px",
  ...
  return (
    <div style={wrap}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingBottom: 12, borderBottom: "1px dashed #2a2e4a" }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: 13, color: "#fff" }} className="text-gradient">
            <span className="qong-mark">QONG</span> Drive
          </div>
          <div style={cap}>P&amp;ID Intelligence v1.0</div>
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          <span style={tagPill("#22D3EE")}>SCANNING</span>
          <span style={tagPill("#8B3FCE")}>
            <span className="qong-mark">QONG</span>-PID-001 · R3
          </span>
        </div>
      </div>
      ...
      <div style={{ marginTop: 14 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
          <div style={{ padding: "8px 10px", background: "#1a1d35", borderRadius: 6, border: "1px solid #2a2e4a", fontFamily: "var(--font-mono)", fontSize: 11 }}>
            <span style={{ color: "#22D3EE", fontWeight: 700 }}>V-102</span>{" "}
            <span style={{ color: "#9498AE" }}>Gate · 4&quot; · CS</span>
          </div>
          <div style={{ padding: "8px 10px", background: "#1a1d35", borderRadius: 6, border: "1px solid #2a2e4a", fontFamily: "var(--font-mono)", fontSize: 11 }}>
            <span style={{ color: "#22D3EE", fontWeight: 700 }}>V-101</span>{" "}
            <span style={{ color: "#9498AE" }}>Ball · 2&quot; · SS</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop: 14, paddingTop: 12, borderTop: "1px dashed #2a2e4a" }}>
        ...
        {([["Instruments", progress.ins], ["Valves", progress.val], ["Pipelines", progress.pip]] as [string, number][]).map(([l, v]) => (
          <div key={l} style={{ marginBottom: 6 }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, marginBottom: 3, color: "#BFC2D2" }}>
              <span>{l}</span>
              <span style={{ fontFamily: "var(--font-mono)", color: "#22D3EE" }}>{v}%</span>
            </div>
