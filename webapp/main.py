...
# Tile line-detection overlay endpoints
from webapp.routers import tile_analysis as tile_analysis_router
# Neo4j Agent Memory — chat API for P&ID jobs
from webapp.routers import agent as agent_router
# inst_field OCR tagging → nearest-symbol propagation
from webapp.routers import bbox_ocr as bbox_ocr_router
# Vendor-match proxy — keeps the vendor API key server-side (FEATURES #124)
...
# Kept per-worker on purpose: migrations are race-safe (existence/column-state
# probes) and MUST complete before this worker serves a request that touches a
# new column. The one-shot side-effects further down are gated to one worker
# (FEATURES #121).
Base.metadata.create_all(bind=engine)
run_migrations()

# Elect one worker to run the one-shot startup side-effects (watchdog, LS
# reconcile, stale-job reset). With uvicorn --workers N these would otherwise
# run N times in parallel — N watchdog threads + N concurrent LS sweeps.
_IS_STARTUP_LEADER = should_run_startup_tasks()
...
# One-shot startup side-effects — only the elected leader runs them so multiple
# uvicorn workers don't duplicate the LS sweep / stale-reset (FEATURES #121).
from webapp.graph.agent_memory import initialize_schema as _init_agent_memory
...
if _IS_STARTUP_LEADER:
    ...
    print("[startup] not the startup leader — skipping stale-reset / super-admin / LS reconcile")
...
# Only the startup leader spawns the reaper thread — N workers would otherwise
# run N reapers contending over the same JobRuns (FEATURES #121).
if _IS_STARTUP_LEADER:
    start_watchdog(app)
...
app.include_router(tile_analysis_router.router)
# Agent memory chat endpoints (Neo4j Agent Memory Service)
app.include_router(agent_router.router, prefix="/api/v1/jobs")
app.include_router(bbox_ocr_router.router)
