...
from equipment_writer import write_equipment_list
...
def run(pdf_path: str, output_path: str = None, inst_output_path: str = None, datasheet_zip_path: str = None, annotated_pdf_path: str = None, skip_extraction: bool = False, original_filename: str = ""):
    ...
    if not skip_extraction:
        ...
        try:
            raw_instruments, raw_equipment = extract_instruments(
                tiles,
            ...
            if raw_equipment:
                eq_path = str(Path(inst_output_path).parent / "equipment_list.csv") if inst_output_path else None
                print(f"Stage 5d: Writing equipment list ({len(raw_equipment)} items)...")
                write_equipment_list(raw_equipment, eq_path)
        except Exception as e:
